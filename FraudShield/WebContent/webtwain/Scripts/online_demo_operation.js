...
function btnUpload_onclick() {
    ...
    var strSessionId, strIdDocType, strIdDocExpDate, strIdDocDesc;
    
    strSessionId = document.getElementById("sessionID").value;
    strIdDocType =  document.getElementById("idDocType").value;
    strIdDocExpDate = document.getElementById("idDocExpiration").value.replace(/\//g, '');
    strIdDocDesc =  document.getElementById("idDocOtherDesc").value;
    ...
    strActionPage = CurrentPath + "SaveToFile.jsp"; //the ActionPage's file path , Online Demo:"SaveToDB.aspx" ;Sample: "SaveToFile.aspx";
    strActionPage = strActionPage + "?sid=" + strSessionId + "&docType=" + strIdDocType + "&docExpDate=" + strIdDocExpDate + "&docDesc=" + strIdDocDesc;
	...
	var fFun = function(){
		var response = DWObject.HTTPPostResponseString;
		response = response.trim();
		if(response.indexOf('FILE_TOO_LARGE') >= 0) {
			alert("Image Uploaded Failed, file is too large. Maxium size 1MB");			
		}else {
			alert("Image Uploaded Successfully");
			location.reload(true);
		}
